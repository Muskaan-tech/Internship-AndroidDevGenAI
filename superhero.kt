...
// 3) Filter heroes by power (case-insensitive)
fun filterByPower(heroes: List<Superhero>, powerQuery: String): List<Superhero> {
    return heroes.filter {
        it.power.toLowerCase().contains(powerQuery.toLowerCase())
    }
}
...
fun main() {
    ...
    val heroes = listOf(
        ...
        Superhero(
            ...
            "Energy Projection",
            false,
            "Hal Jordan, alias Green Lantern, power Energy Projection"
        )
    )
